...
pheno_geno <- function (df, AST, gene_list, antibio) {
  ...
  nb_r_strains <- df %>% filter (!!AST == 1) %>% count() %>% pull(n)
  cat(nb_r_strains, ' strains are resistant to ', antibio , ' (', AST_name, ')')
  ...
  # ggsave(plot=p, paste("plots/", AST_name, ".svg", sep=""), width = 14, height = 7, dpi=600)
}
#------------------------------------------------------------------------------
...
df %>% filter( MEM == 1) %>% select(Strain, genus, ACT_2, Salm_gyrA, Ecoli_marR, Ecoli_acrR, sul2, KpnEF)


# look at CCV in details
df %>% filter( CCV == 1) %>% select(Strain, genus, CCV, Ecoli_acrR, H_NS, marA, Ecoli_marR, Ecoli_ampC1_bl, CTX_M_1, TolC, Ecoli_ampC_bl, KpnEF)
